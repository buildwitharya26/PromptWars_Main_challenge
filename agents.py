...
class WeatherResponse(BaseModel):
    temperature: str = Field(
        ..., description="Current temperature and unit (e.g. '72°F' or '22°C')"
    )
    conditions: str = Field(
        ..., description="Weather conditions (e.g. 'Sunny', 'Rainy', 'Snowy')"
    )
    best_time_to_visit: str = Field(
        ..., description="Best season or month to visit this destination"
    )
    travel_tips: str = Field(
        ...,
        description="Weather-specific travel advice (e.g. 'pack an umbrella', 'wear sunscreen')",
    )


class Activity(BaseModel):
    time: str = Field(
        ..., description="Suggested time range, e.g. '09:00 AM - 11:30 AM'"
    )
...
class DayItinerary(BaseModel):
    ...
    local_tips: List[str] = Field(
        ..., description="Tips specific to the day's locations/activities"
    )
    transportation: str = Field(
        ..., description="Best way to travel between these locations"
    )
...
class Hotel(BaseModel):
    ...
    description: str = Field(
        ..., description="Brief description highlighting key amenities/location"
    )
    price_range: str = Field(
        ..., description="Approximate price per night (e.g., '$100 - $150')"
    )
...
class Restaurant(BaseModel):
    ...
    description: str = Field(
        ..., description="Brief description/vibe of the restaurant"
    )
    must_try_dishes: List[str] = Field(
        ..., description="List of signature or must-try dishes"
    )
    specialty: str = Field(
        ..., description="Type of cuisine or specialty (e.g., 'French Bistro', 'Sushi')"
    )


class HotelFoodResponse(BaseModel):
    hotels: List[Hotel] = Field(
        ..., description="Recommended accommodations across budget tiers"
    )
...
class CostBreakdown(BaseModel):
    ...
    local_transport: str = Field(
        ..., description="Estimated local transport cost (taxi, metro, etc.)"
    )
...
class BudgetResponse(BaseModel):
    currency: str = Field(
        ...,
        description="Currency used for calculations (usually USD or local currency)",
    )
    budget_tier: CostBreakdown = Field(
        ..., description="Cost breakdown for a budget trip"
    )
    mid_range_tier: CostBreakdown = Field(
        ..., description="Cost breakdown for a mid-range trip"
    )
    luxury_tier: CostBreakdown = Field(
        ..., description="Cost breakdown for a luxury/premium trip"
    )
...
class PackingItem(BaseModel):
    ...
    quantity: str = Field(
        ..., description="Suggested quantity (e.g., '3 pairs', '1 jacket', 'as needed')"
    )
...
class PackingResponse(BaseModel):
    ...
    documents: List[PackingItem] = Field(
        ..., description="Passports, visas, reservations, IDs"
    )
    electronics: List[PackingItem] = Field(
        ..., description="Chargers, adapters, devices"
    )
    medicines: List[PackingItem] = Field(
        ..., description="First aid, prescriptions, toiletries"
    )
    accessories: List[PackingItem] = Field(
        ..., description="Bags, umbrellas, sunglasses, and other misc items"
    )
...
class TravelPlanRequest(BaseModel):
    destination: str = Field(
        ..., min_length=1, max_length=100, description="Target destination"
    )
    ...
    interests: List[str] = Field(
        default_factory=list, description="List of travel interests"
    )
...
class WeatherAgent(BaseAgent):
    ...
    async def generate(self, destination: str) -> WeatherResponse:
        ...
        return await self.generate_structured(
            prompt, WeatherResponse, system_instruction
        )
...
class ItineraryAgent(BaseAgent):
    ...
    async def generate(
        ...
        logger.info(
            f"ItineraryAgent generating {days}-day plan for {destination} (Budget: {budget})"
        )
        ...
        return await self.generate_structured(
            prompt, ItineraryResponse, system_instruction
        )
...
class HotelAndFoodAgent(BaseAgent):
    ...
    async def generate(self, destination: str) -> HotelFoodResponse:
        ...
        return await self.generate_structured(
            prompt, HotelFoodResponse, system_instruction
        )
...
class BudgetAgent(BaseAgent):
    ...
    async def generate(self, destination: str, days: int) -> BudgetResponse:
        ...
        return await self.generate_structured(
            prompt, BudgetResponse, system_instruction
        )
...
class PackingAgent(BaseAgent):
    ...
    async def generate(
        self, destination: str, days: int, weather_conditions: str = ""
    ) -> PackingResponse:
        logger.info(
            f"PackingAgent compiling list for: {destination} ({days} days, Weather: {weather_conditions})"
        )
        ...
        return await self.generate_structured(
            prompt, PackingResponse, system_instruction
        )
...
class OrchestratorIntent(BaseModel):
    action: str = Field(
        ..., description="Action to take: 'CREATE_PLAN', 'MODIFY_PLAN', or 'CHAT'"
    )
    destination: Optional[str] = Field(
        None, description="The destination extracted or inferred"
    )
    days: Optional[int] = Field(
        None,
        description="Duration in days, default to 5 if not specified but destination is present",
    )
    budget: Optional[str] = Field(
        None, description="Budget level: 'Budget', 'Mid-range', or 'Luxury'"
    )
    interests: List[str] = Field(
        default_factory=list,
        description="Extracted user interests (e.g. ['food', 'museums'])",
    )
    travelers: Optional[int] = Field(1, description="Number of travelers, default to 1")
    modification_instructions: Optional[str] = Field(
        None, description="Instructions on what to change in the existing plan"
    )
    conversational_reply: Optional[str] = Field(
        None, description="Direct friendly text reply if the action is CHAT"
    )
...
class TravelOrchestratorAgent(BaseModel):
    ...
    async def _determine_intent(
        self, message: str, history: List[Dict[str, Any]], has_existing_plan: bool
    ) -> OrchestratorIntent:
        ...
        if response.text is None:
            raise ValueError("Model returned empty text for intent analysis")
        return OrchestratorIntent.model_validate_json(response.text)

    async def chat(
        self, message: str, session_id: str, destination: str = ""
    ) -> Dict[str, Any]:
        """Main entry point for user chat interaction."""
        logger.info(
            f"Orchestrator chat. Session: {session_id}, Destination Override: '{destination}'"
        )
        ...
        intent = await self._determine_intent(
            message, history, existing_plan is not None
        )
        logger.info(
            f"Orchestrated intent: {intent.action} (Destination: {intent.destination})"
        )
        ...
        if (
            intent.action == "CREATE_PLAN" or target_destination
            ...
            try:
                ...
                itinerary_task = self.itinerary_agent.generate(
                    dest, days, budget, interests, travelers
                )
                ...
                reply = summary_res.text.strip() if summary_res.text is not None else ""
                ...
                return {"reply": err_msg, "plan": None, "session_id": session_id}
            ...
            try:
                ...
                if itinerary_res.text is None:
                    raise ValueError("Model returned empty text for itinerary revision")
                revised_itinerary = ItineraryResponse.model_validate_json(
                    itinerary_res.text
                )
                ...
                err_msg = (
                    f"I tried to update your plan but encountered an error: {str(e)}"
                )
                self.session_service.add_message(session_id, "assistant", err_msg)
                return {
                    "reply": err_msg,
            ...
            reply = (
                ...
                or "How can I help you plan your next adventure?"
            )
            self.session_service.add_message(session_id, "assistant", reply)
            ...
            return {"reply": reply, "plan": plan_dump, "session_id": session_id}
