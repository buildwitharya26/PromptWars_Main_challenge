    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Geocoding API returned status {response.status_code} for {destination}")
...
async def get_us_weather(latitude: float, longitude: float) -> str:
    ...
    async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
        try:
            ...
            if response.status_code != 200:
                logger.error(f"api.weather.gov points failed: {response.status_code} {response.text}")
            ...
            if not forecast_url:
                logger.error(f"No forecast URL found in weather.gov points response: {data}")
                return "US weather details currently unavailable (forecast URL missing)."
            ...
            if forecast_response.status_code != 200:
                logger.error(f"api.weather.gov forecast failed: {forecast_response.status_code}")
            ...
            weather_summary = []
            for period in periods[:3]:  # Capture first 3 periods (e.g. Today, Tonight, Tomorrow)
